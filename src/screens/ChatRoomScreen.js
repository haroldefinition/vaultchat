...
// Uses Socket.io via src/services/socket.js — no REST polling
...
// ── Message bubble ────────────────────────────────────────────
function Bubble({ item, myId, tx, sub, card, accent, onFullScreen, onPlay, onLongPress }) {
  const me    = item.senderId === myId;
  const raw   = item.content || '';
  const nlIdx = raw.indexOf('\n');
  const main  = nlIdx >= 0 ? raw.substring(0, nlIdx) : raw;
  const cap   = nlIdx >= 0 ? raw.substring(nlIdx + 1).trim() : '';

  const isMedia = main.startsWith('GALLERY:') || main.startsWith('LOCALIMG:')
               || main.startsWith('IMG:')      || main.startsWith('VIDEOS:')
               || main.startsWith('LOCALVID:') || main.startsWith('VID:');
  ...
  const bubbleStyle = [s.bubble, me ? s.myBubble : [s.theirBubble, { backgroundColor: card }], isMedia && s.mediaPad];
  ...
  const body = () => {
      ...
      return (
        <>
          <View style={[s.replyQ, { borderLeftColor: me ? 'rgba(255,255,255,0.5)' : accent }]}>
            ...
            <Text style={[s.replyTx, { color: me ? 'rgba(255,255,255,0.55)' : sub }]} numberOfLines={2}>{raw.substring(6, p)}</Text>
          </View>
          <Text style={[s.msgTx, { color: me ? '#fff' : tx }]}>{raw.substring(p + 1)}</Text>
        </>
      );
    }
    if (main.startsWith('GALLERY:')) {
      return <>
        <ResolvedPhotoStack keys={main.replace('GALLERY:', '').split('|')} onLongPress={onLongPress} />
        {cap ? <Text style={[s.cap, { color: me ? 'rgba(255,255,255,0.9)' : tx }]}>{cap}</Text> : null}
      </>;
    }
    if (main.startsWith('LOCALIMG:') || main.startsWith('IMG:')) {
      return <>
        <SinglePhoto msgKey={main.replace('LOCALIMG:', '').replace('IMG:', '')} isLocal={main.startsWith('LOCALIMG:')} onFullScreen={onFullScreen} onLongPress={onLongPress} />
        {cap ? <Text style={[s.cap, { color: me ? 'rgba(255,255,255,0.9)' : tx }]}>{cap}</Text> : null}
      </>;
    }
    ...
    if (main.startsWith('LOCALVID:') || main.startsWith('VID:')) {
      return <>
        <VideoBubble uri={main.replace('LOCALVID:', '').replace('VID:', '')} onPlay={onPlay} onLongPress={onLongPress} />
        {cap ? <Text style={[s.cap, { color: me ? 'rgba(255,255,255,0.9)' : tx }]}>{cap}</Text> : null}
      </>;
    }
    if (main.startsWith('FILE:')) {
      ...
      return (
        <TouchableOpacity style={{ flexDirection: 'row', alignItems: 'center', gap: 10 }} onPress={() => url && Linking.openURL(url)} onLongPress={onLongPress} delayLongPress={450}>
          ...
          <View><Text style={[s.msgTx, { color: me ? '#fff' : tx }]}>{fname}</Text><Text style={{ fontSize: 11, color: me ? 'rgba(255,255,255,0.6)' : sub }}>Tap to open</Text></View>
        </TouchableOpacity>
      );
    }
  ...
  return (
    <View style={[s.bWrap, me ? s.myWrap : s.theirWrap]}>
      <TouchableOpacity style={bubbleStyle} onLongPress={onLongPress} delayLongPress={450} activeOpacity={0.88}>
        {body()}
      </TouchableOpacity>
      <Text style={[s.time, me ? s.tR : s.tL]}>{ts}</Text>
    </View>
  );
}

// ── Compose area for staged media ────────────────────────────
function ComposeArea({ stagedPhotos, stagedVideos, text, setText, onSendPhotos, onSendVideos, onRemovePhoto, onAddMore, onClearVideos, sending, sub, tx, inputBg, border, accent }) {
  return (
    <View>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ gap: 6, padding: 10 }}>
            {stagedPhotos.map((p, i) => (
              <View key={i} style={{ position: 'relative' }}>
                ...
                <TouchableOpacity style={{ position: 'absolute', top: -7, right: -7, width: 22, height: 22, borderRadius: 11, backgroundColor: '#ff3b30', alignItems: 'center', justifyContent: 'center', zIndex: 10 }} onPress={() => onRemovePhoto(i)}>
                  <Text style={{ color: '#fff', fontSize: 12, fontWeight: '900', lineHeight: 16 }}>✕</Text>
                </TouchableOpacity>
              </View>
            ))}
            {stagedPhotos.length < 20 && (
              <TouchableOpacity style={{ width: 90, height: 90, borderRadius: 12, borderWidth: 1.5, borderStyle: 'dashed', borderColor: border, backgroundColor: inputBg, alignItems: 'center', justifyContent: 'center' }} onPress={onAddMore}>
      ...
      {stagedVideos.length > 0 && (
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 12, padding: 12, backgroundColor: inputBg, marginHorizontal: 12, borderRadius: 14, marginBottom: 4 }}>
          ...
          <TouchableOpacity onPress={onClearVideos} style={{ padding: 8 }}><Text style={{ color: sub, fontSize: 18 }}>✕</Text></TouchableOpacity>
        </View>
      )}
      <View style={{ flexDirection: 'row', alignItems: 'center', paddingHorizontal: 12, paddingVertical: 10, paddingBottom: 24, gap: 10, borderTopWidth: 1, borderTopColor: border }}>
        <TextInput style={{ flex: 1, paddingHorizontal: 14, paddingVertical: 10, borderRadius: 22, fontSize: 15, maxHeight: 80, minHeight: 42, backgroundColor: inputBg, color: tx }} placeholder="Add a caption… (optional)" placeholderTextColor={sub} value={text} onChangeText={setText} multiline />
        <TouchableOpacity style={{ width: 44, height: 44, borderRadius: 22, alignItems: 'center', justifyContent: 'center', backgroundColor: accent }} onPress={() => { if (stagedVideos.length > 0) onSendVideos(); else onSendPhotos(); }} disabled={sending}>
...
export default function ChatRoomScreen({ route, navigation }) {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (raw) {
        ...
        joinRoom(roomId, id);
        // Load history from room:history event
        sock.off('room:history');
        sock.on('room:history', ({ roomId: rId, messages: hist }) => {
          if (rId === roomId) {
            setMessages(hist || []);
        ...
        sock.on('message:received', (msg) => {
          if (msg.roomId === roomId) {
            setMessages(prev => {
              const already = prev.find(m => m.messageId === msg.messageId);
  ...
  function emit(content) {
    const sock = getSocket();
    if (!sock) { Alert.alert('Not connected', 'Reconnecting…'); return false; }
    const payload = {
      roomId,
      messageId:   `msg_${Date.now()}`,
      senderId:    myId,
      senderName:  contactPhone || myId,
      content,
      type:        'text',
  ...
  return (
    <KeyboardAvoidingView style={[s.container, { backgroundColor: bg }]} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
      ...
      <View style={[s.header, { backgroundColor: card, borderBottomColor: border }]}>
        ...
        <TouchableOpacity style={[s.hAvatar, { backgroundColor: accent }]}>
          {contactPhoto ? <Image source={{ uri: contactPhoto }} style={s.hAvatarImg} /> : <Text style={s.hAvatarTx}>{(contactName || '?')[0]?.toUpperCase()}</Text>}
        </TouchableOpacity>
        <View style={{ flex: 1 }}>
          <Text style={[s.hName, { color: tx }]}>{contactName || contactPhone || 'Chat'}</Text>
          <Text style={[s.hSub, { color: sub }]}>🛡️ End-to-end encrypted</Text>
        </View>
        <TouchableOpacity onPress={() => navigation.navigate('ActiveCall', { recipientName: contactName, recipientPhone: contactPhone, callType: 'video' })} style={s.callBtn}><Text style={{ fontSize: 20 }}>📹</Text></TouchableOpacity>
        <TouchableOpacity onPress={() => navigation.navigate('ActiveCall', { recipientName: contactName, recipientPhone: contactPhone, callType: 'voice' })} style={s.callBtn}><Text style={{ fontSize: 20 }}>📞</Text></TouchableOpacity>
      </View>
      ...
      {/* Compose or input bar */}
      {hasStaged ? (
        <ComposeArea
          stagedPhotos={stagedPhotos} stagedVideos={stagedVideos}
          text={text} setText={setText}
          onSendPhotos={sendStagedPhotos} onSendVideos={sendStagedVideos}
          onRemovePhoto={i => setStagedPhotos(prev => prev.filter((_, j) => j !== i))}
          onAddMore={() => handleAttachType('photo')}
          onClearVideos={() => setStagedVideos([])}
          sending={sending} sub={sub} tx={tx} inputBg={inputBg} border={border} accent={accent}
        />
      ) : (
            ...
            <View style={s.attachGrid}>
              {ATTACHMENTS.map((a, i) => (
                <TouchableOpacity key={i} style={s.attachItem} onPress={() => pickAttach(a.type)}>
                  <View style={[s.attachIcon, { backgroundColor: inputBg }]}><Text style={{ fontSize: 28 }}>{a.icon}</Text></View>
        ...
        <View style={s.overlay}>
          <View style={[s.sheet, { backgroundColor: card, maxHeight: '60%' }]}>
            ...
            <View style={s.gifGrid}>
              {GIFS.map((g, i) => (
                <TouchableOpacity key={i} style={[s.gifItem, { backgroundColor: inputBg }]} onPress={() => { setGifModal(false); sendText(g.m); }}>
                  <Text style={{ fontSize: 32 }}>{g.e}</Text>
                </TouchableOpacity>
              ))}
            </View>
        ...
        <View style={s.overlay}>
          <View style={[s.sheet, { backgroundColor: card, maxHeight: '65%' }]}>
            ...
            <View style={[s.tabRow, { backgroundColor: inputBg }]}>
              {['emoji', 'gif'].map(t => (
                <TouchableOpacity key={t} style={[s.tab, emojiTab === t && { backgroundColor: card }]} onPress={() => setEmojiTab(t)}>
            ...
            <ScrollView showsVerticalScrollIndicator={false}>
              {emojiTab === 'gif' ? (
                <View style={s.gifGrid}>
                  {GIFS.map((g, i) => (
                    <TouchableOpacity key={i} style={[s.gifItem, { backgroundColor: inputBg }]} onPress={() => { setEmojiModal(false); sendText(g.m); }}>
                      <Text style={{ fontSize: 32 }}>{g.e}</Text>
                    </TouchableOpacity>
                  ))}
                </View>
                ...
                <View style={s.emojiGrid}>
                  {EMOJIS.map((e, i) => (
                    <TouchableOpacity key={i} style={[s.emojiItem, { backgroundColor: inputBg }]} onPress={() => { setEmojiModal(false); sendText(e); }}>
...
const s = StyleSheet.create({
  ...
  cap:        { fontSize: 14, lineHeight: 20, paddingHorizontal: 6, paddingTop: 5, paddingBottom: 2 },
